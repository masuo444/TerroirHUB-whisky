    ...
    return f'''
        <article class="fcard" data-kw="{kw}" data-price="{price or 0}" data-band="{band}">
          ...
          <div class="fcard-body">
            <a class="fcard-winery" href="/whisky/{esc(it.get('pref',''))}/{esc(it.get('winery_id','').split(':')[-1])}.html">{esc(it['winery'])} ›</a>
            <h3 class="fcard-name" title="{esc(it['name'])}">{esc(nm)}</h3>
            {price_html}
            <a class="fcard-btn" href="{esc(it['url'])}" target="_blank" rel="nofollow sponsored noopener" onclick="if(window.gtag)gtag('event','furusato_click',{{genre:'whisky'}});">楽天ふるさと納税で寄付 →</a>
          </div>
        </article>'''
...
FAQ_COMMON = [
    ("ふるさと納税でウイスキーを選ぶメリットは？",
     "実質自己負担2,000円で、寄付額に応じた返礼品としてウイスキーを受け取れます。さらに地域の小規模蒸溜所を直接応援できるのが魅力です。控除上限額は年収・家族構成で変わるため、各ふるさと納税サイトのシミュレーターで確認してから寄付しましょう。"),
    ("掲載のウイスキーはどこで寄付できますか？",
...
<meta property="og:image" content="https://whisky.terroirhub.com/img/hero.jpg">
...
def build_hub(by_pref):
    ...
    budget_cards = "".join(
        f'<a class="bs-card" href="/whisky/furusato/budget-{_k}.html">'
    ...
    faqs = [("ウイスキーのふるさと納税でおすすめは？",
             f"Terroir HUB WHISKYでは全国{n_pref}都道府県・{n_winery}蒸溜所のウイスキーふるさと納税返礼品{total}件を産地別に掲載。山梨・長野・北海道・山形など主要産地から、純米・吟醸・大吟醸など多彩なウイスキーを寄付で応援しながら受け取れます。")] + FAQ_COMMON
    desc = (f"ウイスキーのふるさと納税返礼品を産地別に{total}件掲載。全国{n_winery}蒸溜所の純米・吟醸・大吟醸を、"
...
<header class="hero">
  <div class="hero-inner">
    ...
    <h1>寄付して、日本の蒸溜所を応援しよう。</h1>
    <p>ふるさと納税の返礼品としてウイスキーを受け取りながら、産地と蔵を直接応援。Terroir HUBが全国の蒸溜所の返礼品を産地別に集めました。実質自己負担2,000円で、あなたの好きなウイスキーを。</p>
    <div class="stats">
      <div class="stat"><div class="stat-n">{total}</div><div class="stat-l">返礼品</div></div>
      <div class="stat"><div class="stat-n">{n_winery}</div><div class="stat-l">蒸溜所</div></div>
...
<section class="steps"><div class="steps-grid">
  ...
  <div class="step"><div class="step-n">2</div><h3>好きなウイスキーを選ぶ</h3><p>産地・蒸溜所から選んで「楽天ふるさと納税で見る」へ。寄付すると返礼品として届きます。</p></div>
...
<div class="toolbar"><div class="toolbar-inner">
  <input type="search" class="search-box" id="fsearch" placeholder="🔍 蒸溜所名・銘柄で絞り込む（例: 獺祭、純米大吟醸、新潟）">
...
def build_pref(pref, items, all_prefs):
    ...
    lead = PREF_LEAD.get(pref) or (
        ...
        + f"{('、'.join(wineries[:3]))}など{n_winery}の蒸溜所の返礼品から、寄付で{pn}の地酒と蒸溜所を直接応援できます。")
    cards = "".join(card_html(it) for it in items)

    title = f"{pn}のウイスキー ふるさと納税 返礼品{n}件 | Terroir HUB WHISKY"
    desc = (f"{pn}のウイスキーふるさと納税返礼品を{n}件掲載（{n_winery}蒸溜所）。"
    ...
    faqs = [
        (f"{pn}のウイスキーでふるさと納税のおすすめは？",
         f"{pn}では{n_winery}の蒸溜所から{n}件のウイスキー返礼品が選べます。{('、'.join(wineries[:5]))}などのウイスキーが寄付で受け取れます。"),
...
<main class="content">
  ...
  <div class="otherpref">
    ...
    <div class="otherpref-list">
      <a href="/whisky/{pref}/">{esc(pn)}の蒸溜所一覧</a>
      <a href="/whisky/guide/">ウイスキーの基礎知識</a>
      <a href="/whisky/search/">蒸溜所検索</a>
    </div>
  </div>
  <div class="otherpref">
...
def build_budget(band_key, band_label, by_pref, all_prefs):
    ...
    「ふるさと納税 ウイスキー 1万円以下」のような金額指定の検索にも対応する。"""
    ...
    url = f"https://{DOMAIN}/whisky/furusato/{slug}.html"
    title = f"{band_label}のウイスキー ふるさと納税 返礼品{n}件 | Terroir HUB WHISKY"
    desc = (f"寄付金額{band_label}のウイスキーふるさと納税返礼品を{n}件掲載（{n_pref}都道府県・{n_winery}蒸溜所）。"
            f"寄付額{lo:,}円〜{hi:,}円の返礼品を金額順に並べています。楽天ふるさと納税対応。")
    lead = (f"寄付金額が{band_label}のウイスキー返礼品を{n}件集めました（{n_pref}都道府県・{n_winery}蒸溜所）。"
            f"控除上限額の範囲に収まるものを選べます。金額は掲載時点の楽天ふるさと納税の情報です。")
    crumb = [("Terroir HUB WHISKY", f"https://{DOMAIN}/"),
             ("ふるさと納税", f"https://{DOMAIN}/whisky/furusato/"),
             (f"{band_label}", url)]
    faqs = [
        (f"{band_label}のウイスキー返礼品は何件ありますか？",
         f"Terroir HUB WHISKYでは{band_label}のウイスキー返礼品を{n}件掲載しています（{n_pref}都道府県・{n_winery}蒸溜所）。"),
        ("寄付金額はどこの情報ですか？",
         "楽天ふるさと納税の掲載金額をもとにしています。金額・在庫・取扱自治体は変動するため、寄付前に各返礼品ページで最新の内容を確認してください。"),
    ] + FAQ_COMMON[:3]
    jsonld = jsonld_for(f"{band_label}のウイスキー ふるさと納税返礼品", desc, url, crumb, items, faqs)
    head = HEAD.format(title=esc(title), desc=esc(desc),
                       ogtitle=esc(f"{band_label}のウイスキー ふるさと納税 返礼品{n}件"),
                       canonical=url, jsonld=jsonld) + HEAD_TAIL

    others = "".join(
        f'<a href="/whisky/furusato/budget-{k}.html">{esc(lb)}</a>'
        for k, lb, _lo, _hi in BUDGET_BANDS if k != band_key)
    pref_links = "".join(
        f'<a href="/whisky/furusato/{p}.html">{esc(PREF_NAMES[p])}</a>' for p in all_prefs)
    cards = "".join(card_html(it) for it in items)
    faq_html = "".join(f'<div class="faq-item"><dt>{esc(q)}</dt><dd>{esc(a)}</dd></div>' for q, a in faqs)

    body = f'''
<div class="crumb"><a href="/">ホーム</a> › <a href="/whisky/furusato/">ふるさと納税</a> › {esc(band_label)}</div>
<header class="hero">
  <div class="hero-inner">
    <div class="hero-tag">FURUSATO TAX — 寄付金額 {esc(band_label)}</div>
    <h1>{esc(band_label)}のウイスキーを、ふるさと納税で。</h1>
    <p>{esc(lead)}</p>
    <div class="stats">
      <div class="stat"><div class="stat-n">{n}</div><div class="stat-l">返礼品</div></div>
      <div class="stat"><div class="stat-n">{n_winery}</div><div class="stat-l">蒸溜所</div></div>
...
<main class="content">
  <section class="pref-block">
    <h2 class="pref-title">{esc(band_label)}のウイスキー返礼品 <span class="pref-count">{n}件</span></h2>
    <div class="fgrid">{cards}
    </div>
  </section>
  <div class="otherpref">
    <h2>ほかの寄付金額で見る</h2>
    <div class="otherpref-list">{others}<a href="/whisky/furusato/">すべての返礼品</a></div>
  </div>
  <div class="otherpref">
    <h2>都道府県から探す</h2>
