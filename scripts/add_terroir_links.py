#!/usr/bin/env python3
"""県一覧ページ（whisky/{pref}/index.html）にポータルの県別テロワールページへの導線を追加。

/terroir/ 48ページがジャンルサイトから被リンクゼロで孤立していたための内部リンク施策（2026-07-17）。
sake版 TerriorHUB sake/scripts/add_terroir_links.py を参考に移植。
冪等: 既に terroirhub.com/terroir/ へのリンクがあるページはスキップ。
日本語の県一覧ページ（whisky/{pref}/index.html）のみ対象。英語版（whisky/en/{pref}/）は
そもそも県一覧ページが存在しないため対象外。
"""
import os
...
def block(pref, name):
    ...
    return (f'<div style="max-width:1080px;margin:36px auto 0;padding:0 24px;text-align:center;">'
            f'<a href="{url}" style="font-size:13px;color:#6B4423;text-decoration:none;letter-spacing:0.03em;">'
...
def main():
    ...
    for pref, name in PREF_JA.items():
        path = os.path.join(BASE, 'whisky', pref, 'index.html')
